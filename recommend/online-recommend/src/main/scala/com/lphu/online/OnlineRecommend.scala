...
object OnlineRecommend {
  ...
  def main(args: Array[String]): Unit = {
    val config = Map(
      ...
      "kafka.topic" -> "recommend_logss",
      "rating.prefix" -> "PRODUCT_RATING_PREFIX:"
    )

    // 创建spark conf
    val sparkConf = new SparkConf().setMaster(config("spark.cores")).setAppName("OnlineRecommend")
    ...
    ratingStream.foreachRDD {
      rdds => rdds.foreach {
        case (userId, productId, score, timestamp) =>
          ...
          val userRecentlyRatings = getUserRecentlyRatings(MAX_USER_RATING_NUM, userId, ConnectHelper.jedis)
  ...
  // key userId:userId, value list productId:Score
  ...
  def getTopSimProducts(num: Int,
    ...
    val ratingCollection = ConnectHelper.mongoClient( mongoConfig.db )( MONGODB_RATING_COLLECTION )
  ...
  : Array[(Int, Double)] ={
    ...
    // 返回推荐列表，按照得分排序
    scores.toArray.sortWith(_._2 > _._2)
  }
  ...
  def saveDataToMongoDB(userId: Int, streamRecs: Array[(Int, Double)])(implicit mongoConfig: MongoConfig): Unit ={
    val streamRecsCollection = ConnectHelper.mongoClient(mongoConfig.db)(STREAM_RECS)
    // 按照userId查询并更新
